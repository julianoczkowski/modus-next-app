...
const overviewPanels = [
  <div className="space-y-3" key="summary">
    ...
    <p className="text-sm text-foreground opacity-80 leading-relaxed">
      Snapshot of deployment health, outstanding tasks, and platform usage trends across
      your Trimble environment.
    </p>
  </div>,
  <div className="space-y-3" key="metrics">
    <div className="text-lg font-semibold text-foreground">Performance metrics</div>
    <p className="text-sm text-foreground opacity-80 leading-relaxed">
      Load charts, processing queues, and pipeline throughput with configurable time ranges.
    </p>
  </div>,
  <div className="space-y-3" key="activity">
    <div className="text-lg font-semibold text-foreground">Recent activity</div>
    <p className="text-sm text-foreground opacity-80 leading-relaxed">
      Audit events for data imports, user sign-ins, and integration triggers over the last 7 days.
    </p>
  </div>,
  <div className="space-y-3" key="settings">
    <div className="text-lg font-semibold text-foreground">Configuration</div>
    <p className="text-sm text-foreground opacity-80 leading-relaxed">
      Manage connectors, notification routing, and environment preferences from a single place.
    </p>
  </div>,
];
...
const variantExamples: Array<{
...
}> = [
  {
    ...
    description: "Classic underline indicator that keeps focus on content-heavy layouts.",
    tabStyle: "bordered",
    tabs: [
      { label: "Overview" },
      { label: "Schedule" },
    ...
    panels: [
      <div key="bordered-0" className="text-sm text-foreground opacity-80 leading-relaxed">
        High-level overview, resource status, and quick actions.
      </div>,
      <div key="bordered-1" className="text-sm text-foreground opacity-80 leading-relaxed">
        Upcoming milestones, crew rotations, and equipment bookings.
      </div>,
      <div key="bordered-2" className="text-sm text-foreground opacity-80 leading-relaxed">
  ...
  {
    title: "Boxed",
    description: "Boxed tabs work well for dashboard navigation or card layouts.",
    tabStyle: "boxed",
    tabs: [
    ...
    panels: [
      <div key="boxed-0" className="text-sm text-foreground opacity-80 leading-relaxed">
        Manage logistics routes for survey crews and equipment deliveries.
      </div>,
      <div key="boxed-1" className="text-sm text-foreground opacity-80 leading-relaxed">
        Access terrain overlays, geofences, and sensor coverage.
      </div>,
      <div key="boxed-2" className="text-sm text-foreground opacity-80 leading-relaxed">
  ...
  {
    title: "Lifted",
    description: "Lifted style surfaces the active tab, ideal for multi-step flows.",
    tabStyle: "lifted",
    tabs: [
      { label: "Step 1" },
      { label: "Step 2" },
    ...
    panels: [
      <div key="lifted-0" className="text-sm text-foreground opacity-80 leading-relaxed">
        Collect baseline data inputs and assign responsibilities.
      </div>,
      <div key="lifted-1" className="text-sm text-foreground opacity-80 leading-relaxed">
        Configure advanced options before validation.
      </div>,
      <div key="lifted-2" className="text-sm text-foreground opacity-80 leading-relaxed">
  ...
  {
    title: "Minimal",
    description: "Remove decoration when tabs act as compact filters or segmented controls.",
    tabStyle: "none",
    tabs: [
      { label: "All" },
      { label: "In progress" },
    ...
    panels: [
      <div key="none-0" className="text-sm text-foreground opacity-80 leading-relaxed">
        Aggregated list of all tasks and tickets.
      </div>,
      <div key="none-1" className="text-sm text-foreground opacity-80 leading-relaxed">
        Focus on ongoing workstreams requiring attention.
      </div>,
      <div key="none-2" className="text-sm text-foreground opacity-80 leading-relaxed">
...
export default function TabsDemoPage() {
  ...
  const handleTabChange = (detail: { previousTab: number; newTab: number }) => {
    ...
    logEvent(`Switched from tab ${detail.previousTab + 1} to ${detail.newTab + 1}`);
  };
      ...
      <div className="max-w-6xl mx-auto p-8">
        <div className="text-center mb-12">
          ...
          <div className="text-lg text-foreground opacity-80 leading-relaxed max-w-3xl mx-auto">
            Tabs group related views or forms into a single surface. Combine icons, disabled states, and
            controlled tab indices to deliver accessible navigation across Trimble applications.
          </div>
        </div>
        ...
        >
          <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-8 mb-6">
            <div>
              ...
              <p className="text-sm text-foreground opacity-80">
                Manage active tab state within React. Navigation buttons adjust the active index and
                subscribe to `tabChange` for keyboard and click interaction.
              </p>
            </div>
          ...
          <div className="bg-background border border-border rounded-lg p-6">
            ...
            <div className="mt-6 text-sm text-foreground opacity-80 leading-relaxed">
              Active tab:{" "}
              <span className="font-semibold text-foreground">{tabSummary}</span>
            </div>
          </div>
        </div>
        ...
        >
          ...
          <div className="text-sm text-foreground opacity-70 mb-4">
            Records the `tabChange` details emitted from the Modus tabs component when users change sections.
          </div>
